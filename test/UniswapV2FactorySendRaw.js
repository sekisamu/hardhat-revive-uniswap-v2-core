...
let factory;
  ...
  beforeEach(async function () {
    ...
    // const localkey = process.env.LOCAL_PRIV_KEY;
    // const ahkey = process.env.AH_PRIV_KEY;
    const provider = new ethers.JsonRpcProvider(hre.network.config.url);
    const walletKey = hre.network.config.accounts[0];
    wallet = new ethers.Wallet(walletKey, provider);

    // Deploying the pair contract using the factory
    const UniswapV2Pair = require("../artifacts-pvm/contracts/UniswapV2Pair.sol/UniswapV2Pair.json");
    const pairFactory = new ethers.ContractFactory(UniswapV2Pair.abi, UniswapV2Pair.bytecode, wallet);
    pair = await pairFactory.deploy();
    await pair.waitForDeployment();

    // Deploying the pair with raw transaction
    const UniswapV2Pair1 = await ethers.getContractFactory("UniswapV2Pair");
    let bytecode = UniswapV2Pair1.bytecode;
    const rawTx = {
      to: null,
      from: wallet.address,
      data: bytecode,
    ...
    const pairAddress = receipt.contractAddress;
    console.log("pairAddress", pairAddress);

    const ERC20 = require("../artifacts-pvm/contracts/test/ERC20.sol/ERC20.json");
    const erc20Factory = new ethers.ContractFactory(ERC20.abi, ERC20.bytecode, wallet);
    token = await erc20Factory.deploy(TOTAL_SUPPLY);
    await token.waitForDeployment();

    const UniswapV2Factory = require("../artifacts-pvm/contracts/UniswapV2Factory.sol/UniswapV2Factory.json");
    const factoryFactory = new ethers.ContractFactory(UniswapV2Factory.abi, UniswapV2Factory.bytecode, wallet);
    factory = await factoryFactory.deploy(wallet.address);
